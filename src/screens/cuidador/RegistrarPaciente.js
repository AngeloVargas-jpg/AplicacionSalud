...
export default function RegistrarPaciente({ navigation }) {
  ...
  const puedeGuardar = nombre.trim() && rutValido && edad && tipoSangre && pinOk;
  ...
  function guardar() {
    const medsLimpios = meds
      .filter(m => m.nombre.trim())
      .map((m, i) => ({ id: `m${Date.now()}${i}`, ...m, tomado: false }));
    const contactosLimpios = contactos
      .filter(c => c.nombre.trim())
      .map((c, i) => ({ id: `ct${Date.now()}${i}`, ...c }));
    ...
    Alert.alert('✅ Paciente registrado', `${nombre} fue agregado exitosamente.\n\nRUT: ${rut}\nPIN: ${pin}`, [
        ...
        <ScrollView contentContainerStyle={s.body} keyboardShouldPersistTaps="handled">
          ...
          <View style={s.card}>
            ...
            <View style={s.inputWrap}>
              ...
              />
              {rut.length > 3 && <Text style={[s.validIcon, { color: rutValido ? COLORS.verde : COLORS.rojo }]}>{rutValido ? '✓' : '✗'}</Text>}
            </View>
            <View style={s.sep} />
            ...
            <Campo label="Alergias (opcional)" valor={alergias} onChange={setAlergias} placeholder="Ej: Penicilina, AAS..." />
            <View style={s.sep} />
            <Campo label="CESFAM asignado (opcional)" valor={cesfam} onChange={setCesfam} placeholder="Ej: CESFAM Puerto Montt" />
          </View>

          {/* ── PIN ── */}
          <Text style={s.seccion}>🔐 PIN de acceso del adulto mayor</Text>
          <View style={s.card}>
            <Text style={s.hint}>Este PIN lo usará el paciente para entrar a la app junto con su RUT.</Text>
            <View style={s.sep} />
          ...
          <View style={s.card}>
            {meds.map((m, i) => (
              <View key={i}>
                ...
                <TextInput style={s.input} value={m.nombre}
                  onChangeText={v => updateMed(i,'nombre',v)}
                  placeholder="Nombre y dosis — Ej: Losartán 50mg" placeholderTextColor={COLORS.textoSuave} />
                <View style={{ height: 8 }} />
                <View style={s.fila2}>
                  <View style={{ flex: 1 }}>
                    <Text style={s.label}>Hora</Text>
                    <TextInput style={s.input} value={m.hora}
                      onChangeText={v => updateMed(i,'hora',v)}
                      placeholder="Ej: 8:00 AM" placeholderTextColor={COLORS.textoSuave} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={s.label}>Frecuencia</Text>
                    <TextInput style={s.input} value={m.frecuencia}
                      onChangeText={v => updateMed(i,'frecuencia',v)}
            ...
            {contactos.map((c, i) => (
              <View key={i}>
                ...
                <View style={s.filaHeader}>
                  ...
                  {i > 0 && (
                    <TouchableOpacity onPress={() => removeContacto(i)}>
                      <Text style={s.eliminar}>✕ Eliminar</Text>
                    </TouchableOpacity>
                  )}
                </View>
                <TextInput style={s.input} value={c.nombre}
                  onChangeText={v => updateContacto(i,'nombre',v)}
                  placeholder="Nombre y relación — Ej: María (hija)" placeholderTextColor={COLORS.textoSuave} />
                <View style={{ height: 8 }} />
                <TextInput style={s.input} value={c.telefono}
                  onChangeText={v => updateContacto(i,'telefono',v)}
                  placeholder="+56 9 xxxx xxxx" placeholderTextColor={COLORS.textoSuave}
